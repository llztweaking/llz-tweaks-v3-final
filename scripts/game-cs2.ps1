...
if ($proc -and $proc.Path) {
  ...
  if ($priorityOk) {
    Write-Output "CS2 detectado: prioridade alta, GPU dedicada e otimizações de tela cheia aplicadas."
  } else {
    Write-Output "CS2 detectado: GPU dedicada e otimizações de tela cheia aplicadas. Não foi possível elevar a prioridade do processo (permissão negada pelo sistema)."
  }
} else {
  Write-Output "CS2 não está em execução. Ajustes gerais aplicados; abra o jogo e otimize novamente para GPU dedicada e prioridade de processo."
}
