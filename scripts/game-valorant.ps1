...
$proc = Get-Process -Name "VALORANT-Win64-Shipping" -ErrorAction SilentlyContinue | Select-Object -First 1
...
if ($proc -and $proc.Path) {
  ...
  if ($priorityOk) {
    Write-Output "Valorant detectado: prioridade alta, GPU dedicada e otimizações de tela cheia aplicadas."
  } else {
    Write-Output "Valorant detectado: GPU dedicada e otimizações de tela cheia aplicadas. Não foi possível elevar a prioridade do processo (permissão negada pelo sistema)."
  }
} else {
  Write-Output "Valorant não está em execução. Ajustes gerais aplicados; abra o jogo e otimize novamente para GPU dedicada e prioridade de processo."
}
