-- Ajusta a mensagem de login_with_linked_discord: quando a key não existe OU existe mas
-- ainda não foi vinculada a nenhuma conta do Discord, mostra uma mensagem única e mais
-- amigável orientando o cliente a se cadastrar ou conferir os dados — em vez de duas
-- mensagens técnicas diferentes. O caso de key vinculada a OUTRA conta do Discord continua
-- com mensagem própria, já que ali a key está sim cadastrada (só não é dessa conta).
